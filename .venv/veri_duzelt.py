...
def dosya_kurtar():
    print("500 Gerçek Oyuncu verisi işleniyor...")

    # Şampiyonların Oyun İçi Özellikleri (Yapay Zeka için gerekli)
    trait_map = {
    ...
    # Liste formatı: "İsim|Puan|Rol|Şampiyon" (Böylece yer kaplamaz)
    # Senin 500 kişilik listeni buraya sıkıştırdım.
...
Code Lyoko#2010|441|Orta Koridor|Aurelion Sol"""
    ...
    with open("sıralama.txt", "w", encoding="utf-8") as f:
        # Satır satır okuyup, özellik (trait) ekleyip yazacağız
        ...
        for line in lines:
            ...
            if len(parts) >= 4:
                ...
                puan = parts[1].strip()
                rol = parts[3].strip()
                sampiyon = parts[2].strip()

                # Özelliği bul (Yoksa 'Dengeli' ata)
                ozellik = trait_map.get(sampiyon, f"Main: {sampiyon}")

                # CSV Formatında yaz
                f.write(f"{ad}, {puan}, {rol}, {ozellik}\n")

    print(f"[BAŞARILI] {len(lines)} oyuncu 'sıralama.txt' dosyasına kaydedildi!")
    print("Artık ana oyunu çalıştırabilirsin.")
